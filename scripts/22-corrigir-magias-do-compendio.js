/* ============================================================================
 * 22-corrigir-magias-do-compendio.js
 * ----------------------------------------------------------------------------
 * Corretor in-place das magias já importadas no compêndio + em atores.
 * Preserva os IDs atuais (não apaga/recria), então quaisquer referências
 * existentes continuam funcionando.
 *
 * O que faz:
 *   1. Percorre todas as magias do compêndio `tormenta20.magias` e, para cada
 *      uma que tenha correspondência na tabela CORRECTIONS, atualiza
 *      `system.descricao`, `alcance`, `alvo`, `area`, `duracao`, `resistencia`
 *      e `custoPM` com os valores corretos re-extraídos do PDF.
 *   2. Faz o mesmo para magias em World Items e em Actors.
 *   3. Renomeia magias cuja capitalização estava incorreta (ex: "cólera de
 *      azgher" → "Cólera de Azgher").
 *
 * COMO USAR:
 *   Macros → Create Macro → Type: Script
 *   Cole este arquivo INTEIRO e execute. Segue notification de resumo.
 *
 * SEGURANÇA:
 *   - Não apaga nada.
 *   - Idempotente: rodar várias vezes produz o mesmo resultado.
 *   - Recomendado backup do mundo antes.
 *
 * Gerado automaticamente a partir de:
 *   T20 Livro Básico + Deuses de Arton + Heróis de Arton.
 * ==========================================================================*/
...
const CORRECTIONS = {
  ...
  "campeões dos deuses percepção rubra": {
  ...
  "novas magias arcanas armadura elemental": {
...
(async () => {
  const byKey = new Map();
  for (const key in CORRECTIONS) byKey.set(key, CORRECTIONS[key]);

  const keyOf = name => String(name || "").trim().toLowerCase();

  /**
   * Retorna o patch de update para um documento de magia, ou null se não
   * houver correção correspondente nem mudança necessária.
   */
  const buildPatch = (doc) => {
    const corr = byKey.get(keyOf(doc.name));
    if (!corr) return null;

    const upd = {};
    // Renomear se capitalização diferente
    if (doc.name !== corr.name) upd["name"] = corr.name;

    // Comparar e atualizar campos do system
    const sysCorr = corr.system;
    const sysCur  = doc.system || {};
    const fields = [
      "descricao","tipo","circulo","escola","execucao",
      "alcance","alvo","area","duracao","resistencia","custoPM"
    ];
    for (const f of fields) {
      if (sysCorr[f] !== undefined && sysCur[f] !== sysCorr[f]) {
        upd[`system.${f}`] = sysCorr[f];
      }
    }
    return Object.keys(upd).length ? upd : null;
  };

  let compFixed = 0, worldFixed = 0, actorFixed = 0, skipped = 0, missing = [];
  const errors = [];

  // ---- 1) Compêndio ----
  const pack = game.packs.get("tormenta20.magias");
  if (!pack) {
    ui.notifications.warn("Pack 'tormenta20.magias' não encontrado — pulando.");
  } else {
    const wasLocked = pack.locked;
    try {
      if (wasLocked) await pack.configure({ locked: false });
      const docs = await pack.getDocuments();
      console.log(`[T20 correção] Compêndio: ${docs.length} magias encontradas.`);
      for (const doc of docs) {
        try {
          const upd = buildPatch(doc);
          if (upd) { await doc.update(upd); compFixed++; }
          else if (!byKey.has(keyOf(doc.name))) missing.push(doc.name);
          else skipped++;
        } catch (e) {
          console.error(`[T20 correção] erro em ${doc.name}:`, e);
  ...
  for (const it of game.items.contents) {
    ...
    try {
      const upd = buildPatch(it);
      if (upd) { await it.update(upd); worldFixed++; }
    } catch (e) {
      errors.push(`world:${it.name}: ${e.message}`);
    }
  }
  ...
  for (const actor of game.actors.contents) {
    for (const it of actor.items.contents) {
      ...
      try {
        const upd = buildPatch(it);
        if (upd) { await it.update(upd); actorFixed++; }
      } catch (e) {
        errors.push(`${actor.name}/${it.name}: ${e.message}`);
      }
    }
  }

  const total = compFixed + worldFixed + actorFixed;
  console.log(`[T20 correção] Compêndio: ${compFixed} | World: ${worldFixed} | Atores: ${actorFixed} | sem mudança: ${skipped}`);
  if (missing.length) console.warn(`[T20 correção] Magias no compêndio SEM correspondência na tabela (${missing.length}):`, missing);
  if (errors.length)  console.warn(`[T20 correção] Erros:`, errors);

  ui.notifications.info(`✅ ${total} magia(s) corrigida(s). Veja console (F12) para detalhes.`);
})();
